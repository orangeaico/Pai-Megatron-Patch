...
## Vendored Snapshot Provenance (PAI qwen3.5 linear backend)

- Source path: `/home/shramana/training/Megatron-LM`
- Snapshot target: `Pai-Megatron-Patch/backends/megatron/Megatron-LM-qwen35-linear`
- Snapshot timestamp (local): `2026-03-05`
- Source commit: `8b7270df9`
- Source dirty status at snapshot: `dirty` (tracked local edits present)
- Exclusion policy: runtime/cache/build artifacts are excluded from vendored snapshots (for example `.git`, `__pycache__`, virtualenvs, pytest/mypy/tox caches, logs, build/temp outputs, compiled caches).
